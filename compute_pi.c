...
double compute_pi_leibniz_avx2(size_t n)
{
	double pi = 0.0;
	register __m256d ymm0, ymm1, ymm2, ymm3, ymm4;
	register __m256d ymm5, ymm6, ymm7, ymm8, ymm9, ymm10;

	ymm0 = _mm256_setzero_pd();
	ymm1 = _mm256_set_pd(1.0, -1.0, 1.0, -1.0);	
	ymm2 = _mm256_set1_pd(32.0);

	ymm3 = _mm256_set_pd(1.0, 3.0, 5.0, 7.0);
	ymm4 = _mm256_set_pd(9.0, 11.0, 13.0, 15.0);
	ymm5 = _mm256_set_pd(17.0, 19.0, 21.0, 23.0);
	ymm6 = _mm256_set_pd(25.0, 27.0, 29.0, 31.0);	
	
	for (int i = 0; i <= n - 16; i += 16) {
		ymm7 = _mm256_div_pd(ymm1, ymm3);
		ymm8 = _mm256_div_pd(ymm1, ymm4);
		ymm9 = _mm256_div_pd(ymm1, ymm5);
		ymm10 = _mm256_div_pd(ymm1, ymm6);

		ymm0 = _mm256_add_pd(ymm0, ymm7);
		ymm3 = _mm256_add_pd(ymm3, ymm2);		
		ymm0 = _mm256_add_pd(ymm0, ymm8);
		ymm4 = _mm256_add_pd(ymm4, ymm2);		
		ymm0 = _mm256_add_pd(ymm0, ymm9);
		ymm5 = _mm256_add_pd(ymm5, ymm2);
		ymm0 = _mm256_add_pd(ymm0, ymm10);
...
int main(int argc, char* argv[])
{
	...
	switch(operation) {
		...
		case 3:
			...
			break;
		case 4:
			compute_pi = &compute_pi_leibniz_avx2;
			strcpy(method_name, "compute_pi_leibniz_avx2");
			strcpy(time_filename, "time_leibniz_avx2.txt");
			strcpy(error_filename, "error_leibniz_avx2.txt");
			break;
		default:
			break;
	}
